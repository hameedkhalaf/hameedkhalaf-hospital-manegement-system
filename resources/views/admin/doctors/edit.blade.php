...
@section('title')
    {{ trans('Dashboard/doctors.update_doctor') }}
@stop
@endsection
@section('page-header')
<!-- breadcrumb -->
    ...
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto"> <a
                    href="{{ route('doctors.index') }}">{{ trans('Dashboard/main-sidebar_trans.doctors') }}</a> </h4>
            <span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                {{ trans('Dashboard/doctors.update_doctor') }}</span>
        </div>
    </div>
</div>
<!-- breadcrumb -->
                ...
                <form action="{{ route('doctors.update', 'test') }}" method="post" autocomplete="off"
                    ...
                    <div class="pd-30 pd-sm-40 bg-gray-200">
                        <div>
                            @if ($doctor->image)
                                <img style="border-radius:20%"
                                    src="{{ Url::asset('public/img/admin/doctors/' . $doctor->image->filename) }}"
                                    height="150px" width="150px" alt="">
                            @else
                                <img style="border-radius:50%"
                                    src="{{ Url::asset('public/img/admin/doctor_default.png') }}" height="50px"
                                    width="50px" alt="">
                            @endif
                        </div>
                        <br><br>
                        ...
                        <div class="row row-xs align-items-center mg-b-20">
                            <div class="col-md-1">
                                <label for="exampleInputEmail1">
                                    {{ trans('Dashboard/.doctor_photo') }}</label>
                            </div>
                            <div class="col-md-11 mg-t-5 mg-md-t-0">
                        ...
                        <button type="submit"
                            class="btn btn-main-primary pd-x-30 mg-r-5 mg-t-5">{{ trans('Dashboard/doctors.update') }}</button>
                    </div>
                </form>
            </div>
        </div>
